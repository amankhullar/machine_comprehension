...
from modules import RNNEncoder, SimpleSoftmaxLayer, BasicAttn, BiDirectionalAttention, Bidaf
...
class QAModel(object):
    ...
    def build_graph(self):
        ...
        ##attn_layer = BiDirectionalAttention(self.keep_prob, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size)
        ##_, attn_output = attn_layer.build_graph(context_hiddens, self.context_mask, question_hiddens, self.qn_mask) # attn_output is shape (batch_size, context_len, hidden_size*2)

        attn_layer_bidaf = Bidaf(self.keep_prob, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2)
        # Concat attn_output to context_hiddens to get blended_reps
        ##blended_reps = tf.concat([context_hiddens, attn_output], axis=2) # (batch_size, context_len, hidden_size*4)

        _, _, blended_reps = attn_layer_bidaf.build_graph(context_hiddens, self.context_mask, question_hiddens, self.qn_mask) #(batch_size, context_size, 8*hidden_size)
